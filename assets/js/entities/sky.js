var Sky = function(game, x, y, params) {
  ...
  for(var i = 0; this.rows.length * this.width < game.scale.height; i++) {
    var column = [];
    this.rows[i] = column;

    for(var j = 0; column.length * this.width < game.scale.width; j++) {
      var xOffset = 0;
      var yOffset = 0;

      if(j - 1  !== -1) {
        xOffset = j * this.width;
      }

      if(i - 1 !== -1) {
        yOffset = i * this.width;
      }
      ...
      box.drawRect(0, 0, this.width, this.width);
      box.endFill();
      column.push(box);
    }
  }
